...
V2 is a zero-output technical amendment only: the raw GGUF template pin is
separate from the source-normalized template exposed by this runtime's /props.
No template, generation, fixture, scoring rule, or inference budget is changed.
"""
...
SCHEMA = "jbspan-pa-llama-target-admission-v2"
SAFE_ROOT = "data/natural_language_localization/pa_llama_target_admission_v2"
PRIVATE_ROOT = "artifacts/pa_llama_target_admission_v2/private"
RUNNER_PATH = "scripts/run_pa_llama_target_admission_v2.py"
...
def validate_server_args(args):
    ...
    while index < len(args):
        ...
        if key in SERVER_FLAGS:
            ...
            if key == "--cors-origins":
                require(args[index + 1] == "localhost", "CORS_ORIGIN_NOT_LOCALHOST")
            index += 2
...
def load_contract(root, config_path, expected_sha):
    ...
    predecessor = config.get("zero_output_predecessor", {})
    predecessor_sha = predecessor.get("contract_sha256", "")
    require(re.fullmatch(r"[0-9a-f]{64}", predecessor_sha) is not None
            and predecessor.get("combined_generation_ceiling") == 11,
            "ZERO_OUTPUT_PREDECESSOR_REQUIRED")
    abort_spec = predecessor.get("abort_receipt", {})
    predecessor_root = "data/natural_language_localization/pa_llama_target_admission_v1"
    require(abort_spec.get("path") == f"{predecessor_root}/{predecessor_sha}/aborted.safe.json",
            "PREDECESSOR_PATH_INVALID")
    abort = strict_json(descriptor(root, abort_spec).read_bytes())
    require(abort.get("contract_sha256") == predecessor_sha
            and abort.get("error_code") == "NATIVE_TEMPLATE_MISMATCH"
            and type(abort.get("journaled_dispatches")) is int
            and abort["journaled_dispatches"] == 0
            and type(abort.get("reply_receipts")) is int and abort["reply_receipts"] == 0,
            "PREDECESSOR_NOT_ZERO_OUTPUT_TEMPLATE_ABORT")
    require(not list(contained(root, abort_spec["path"]).parent.glob("*.dispatch.safe.json")),
            "PREDECESSOR_HAS_DISPATCH_JOURNALS")
    ...
    for item in runtime["files"]:
        require(item["path"].startswith("artifacts/p2_runtime_qualification_v1/runtime/"),
                "RUNTIME_READ_SCOPE_INVALID")
        descriptor(root, item)
    require(re.fullmatch(r"[0-9a-f]{64}", runtime.get("served_chat_template_sha256", ""))
            is not None, "NATIVE_TEMPLATE_PIN_MISSING")
    require(re.fullmatch(r"[0-9a-f]{64}", runtime.get("native_chat_template_sha256", ""))
            is not None, "RAW_GGUF_TEMPLATE_PIN_MISSING")
    require(type(runtime.get("served_chat_template_utf8_bytes")) is int
            and runtime["served_chat_template_utf8_bytes"] > 0, "SERVED_TEMPLATE_LENGTH_REQUIRED")
...
def check_identity(process, client, config, identity_observer=None):
    ...
    available = isinstance(template, str) and template != ""
    template_sha = sha_bytes(template.encode()) if available else None
    observation = {
        "process_id": process.pid,
        "native_chat_template_sha256": config["runtime"]["native_chat_template_sha256"],
        "expected_served_chat_template_sha256": config["runtime"]["served_chat_template_sha256"],
        "expected_served_chat_template_utf8_bytes": config["runtime"][
            "served_chat_template_utf8_bytes"],
        "observed_served_chat_template_sha256": template_sha,
        "observed_served_chat_template_utf8_bytes": len(template.encode()) if available else None,
        "template_source_available": available,
        "served_template_matches_pin": template_sha == config["runtime"][
            "served_chat_template_sha256"] and available and len(template.encode())
        == config["runtime"]["served_chat_template_utf8_bytes"],
    }
    if identity_observer is not None:
        identity_observer(observation)
    require(available, "SERVED_TEMPLATE_UNAVAILABLE")
    require(template_sha == config["runtime"]["served_chat_template_sha256"],
            "SERVED_TEMPLATE_MISMATCH")
    require(len(template.encode()) == config["runtime"]["served_chat_template_utf8_bytes"],
            "SERVED_TEMPLATE_LENGTH_MISMATCH")
    require(process.poll() is None, "OWNED_PROCESS_EXITED")
    return {"served_chat_template_sha256": template_sha,
        ...
        try:
            ...
            while True:
                ...
                if time.monotonic() - last_update >= 20:
                    ...
                    last_update = time.monotonic()
                time.sleep(0.5)
            def observe_identity(observation):
                write_once(safe_dir / f"server-{epoch:02d}.tpl.safe.json",
                           {**event, **observation})

            properties = check_identity(process, client, config, observe_identity)
...
def dispatch_one(config, contract_sha, item, process, client, safe_dir, private_dir, props):
    ...
    require(process.poll() is None, "OWNED_PROCESS_EXITED")
    def observe_identity(observation):
        write_once(safe_dir / f"{ordinal:02d}.tpl.safe.json", {
            "contract_sha256": contract_sha, "ordinal": ordinal, "epoch": item["epoch"],
            **observation,
        })

    current = check_identity(process, client, config, observe_identity)
    require(current["served_chat_template_sha256"] == props["served_chat_template_sha256"],
    ...
    journal = {"contract_sha256": contract_sha, "ordinal": ordinal, "epoch": item["epoch"],
               ...
               "served_chat_template_sha256": current["served_chat_template_sha256"]}
...
def verify_template_observation(config, contract_sha, path, context):
    require(path.is_file(), "TEMPLATE_OBSERVATION_MISSING")
    observed = strict_json(path.read_bytes())
    runtime = config["runtime"]
    expected = {
    ...
    require(all(identity(observed.get(key)) == identity(value) for key, value in expected.items()),
            "TEMPLATE_OBSERVATION_BINDING_MISMATCH")
...
def reconcile(config, contract_sha, safe_dir, private_dir):
    ...
    for item in plan[:len(journals)]:
        ...
        require(journal.get("contract_sha256") == contract_sha
                ...
                == config["runtime"]["served_chat_template_sha256"], "JOURNAL_BINDING_MISMATCH")
        ...
        verify_template_observation(config, contract_sha, safe_dir / f"{prefix}.tpl.safe.json", {
            "process_id": journal["process_id"], "epoch": item["epoch"],
            "ordinal": item["ordinal"],
        })
...
def summarize(config, contract_sha, rows, *, stopped):
    ...
    same_template = complete and len({row["served_chat_template_sha256"] for row in rows}) == 1
...
def stopped_epochs(config, safe_dir, contract_sha):
    for epoch in (1, 2):
        ...
        require(records[2].get("served_chat_template_sha256")
                == config["runtime"]["served_chat_template_sha256"],
                "SERVER_EPOCH_TEMPLATE_MISMATCH")
        verify_template_observation(config, contract_sha,
                                    safe_dir / f"server-{epoch:02d}.tpl.safe.json", {
                                        "process_id": records[0]["pid"], "epoch": epoch,
                                    })
