...
record = {
    "schema_version": "e1c-harmbench-paired-invariance-worker-result-v1",
    "status": "E1C_HARMBENCH_CANARY_REQUALIFICATION_WORKER_OPERATIONAL_FAIL",
...
path = (
    Path(os.environ["SAFE_DIR"])
    / f"canary_requalification_shard_{int(os.environ['SHARD_INDEX']):02d}_worker_result.safe.json"
)
...
stage="validate_frozen_requalification"
python scripts/run_evaluator_panel_e1c_canary_requalification_live.py validate \
  --root "$ROOT" \
  --config "$CONFIG" \
  --freeze-result "$FREEZE_RESULT" \
  --selection "$SELECTION" \
  --safe-output "$SAFE_DIR/requalification_validation.safe.json"
...
server_args=(
  -m "$model_path"
  --alias harmbench-e1c-canary-requalification
  --host 127.0.0.1
  --port "$server_port"
...
record = {
    "schema_version": "e1c-harmbench-paired-invariance-server-startup-safe-v1",
    "shard_index": int(os.environ["SHARD_INDEX"]),
    "ready": True,
...
path = (
    Path(os.environ["SAFE_DIR"])
    / f"canary_requalification_shard_{int(os.environ['SHARD_INDEX']):02d}_server_startup.safe.json"
)
path.write_text(json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8")
PY
: > "$server_log"

stage="execute_paired_invariance_shard_${SHARD_INDEX}"
inference_performed=true
export INFERENCE_PERFORMED=true
timeout 180m python scripts/run_evaluator_panel_e1c_canary_requalification_live.py \
  full-shard \
  --root "$ROOT" \
