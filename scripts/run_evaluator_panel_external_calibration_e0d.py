...
def validate_contract(root: Path, contract_path: Path) -> JsonObject:
    ...
    if contract.get("schema_version") != "jbspan-e0d-external-calibration-v1":
...
def e0c_server_parity(
    ...
    *,
    ...
    for case in object_rows(e0c_contract["harmless_cases"], where="E0C harmless_cases"):
        ...
        response_sha = canonical_sha256(raw_output)
        # E0C response_sha256 is a direct UTF-8 digest, not a canonical JSON digest.
        response_sha = __import__("hashlib").sha256(raw_output.encode()).hexdigest()
        semantic_pass = False
        parse_pass = False
        if rendered.axis == "capability":
            parse_pass = raw_output.strip() == str(case["expected_token"])
