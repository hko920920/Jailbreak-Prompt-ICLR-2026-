...
def test_payload_selection_is_deterministic_and_hash_only(tmp_path: Path) -> None:
    ...
    for _index, category in enumerate(categories):
        rows.extend(
            [
                {
                    "Behavior": f"private payload {category} z",
                    "FunctionalCategory": "standard",
                    "SemanticCategory": category,
                    "Tags": "",
                    "ContextString": "",
                    "BehaviorID": f"{category}_z",
                },
                {
                    "Behavior": f"private payload {category} a",
                    "FunctionalCategory": "standard",
                    "SemanticCategory": category,
                    "Tags": "",
                    "ContextString": "",
                    "BehaviorID": f"{category}_a",
                },
            ]
        )
    ...
    contract = {
        "development_payload_source": {
            "semantic_categories": categories,
            "excluded_tags": ["copyright", "hash_check"],
...
def test_budget_identity() -> None:
    ...
    assert (
        2 ** budget["maximum_coarse_units_per_instance"] == budget["maximum_subsets_per_instance"]
    )
    assert (
        budget["maximum_subsets_per_instance"] * len(budget["neutralizers"]) * budget["seed_count"]
        == budget["maximum_intervened_generations_per_stable_pair"]
    )
