...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "run_pa_llama_target_admission_v2.py"
SPEC = importlib.util.spec_from_file_location("pa_llama_admission_tested_v2", SCRIPT)
...
@pytest.mark.parametrize(("name", "answer"), [
    ...
    ("arithmetic", "42."), ("arithmetic", "命冥"), ("arithmetic", "42 is correct"),
    ...
    ("copy_token", "alpha??"), ("copy_token", "alpha-7."), ("capital", ""),
])
...
def contract(tmp_path, monkeypatch):
    ...
    abort_receipt = file_spec(
        "data/natural_language_localization/pa_llama_target_admission_v1/"
        f"{predecessor_sha}/aborted.safe.json",
        admission.canonical({"contract_sha256": predecessor_sha,
                             "error_code": "NATIVE_TEMPLATE_MISMATCH",
                             "journaled_dispatches": 0, "reply_receipts": 0}))
    config = {
        "schema_version": admission.SCHEMA, "frozen": True,
        "evidence_class": "HARMLESS_OPERATIONAL_TARGET_ADMISSION_NOT_PAPER_EVIDENCE",
        "zero_output_predecessor": {"contract_sha256": predecessor_sha,
                                    "abort_receipt": abort_receipt,
                                    "combined_generation_ceiling": 11},
        "sources": [], "required_code": {"runner": runner}, "fixture_source": source,
        "fixtures": fixtures,
        "runtime": {"server": runtime, "files": [library],
                    "served_chat_template_sha256": admission.sha_bytes(b"synthetic template"),
                    "native_chat_template_sha256": admission.sha_bytes(b"raw synthetic template"),
                    "served_chat_template_utf8_bytes": len(b"synthetic template"),
...
def install_mock_servers(monkeypatch, contract, response_hook=None):
    ...
    def server(root, config, private_dir, safe_dir, epoch, contract_sha):
        ...
        props = {"served_chat_template_sha256": config["runtime"]["served_chat_template_sha256"]}
        ...
        def observe(observation):
            admission.write_once(safe_dir / f"server-{epoch:02d}.tpl.safe.json",
                                 {**event, **observation})

        admission.check_identity(process, Client(), config, observe)
        try:
...
@pytest.mark.parametrize(("mutate", "code"), [
    ...
    (lambda c: c["runtime"].update(served_chat_template_sha256=""), "NATIVE_TEMPLATE_PIN_MISSING"),
...
@pytest.mark.parametrize("field", ["journaled_dispatches", "reply_receipts"])
def test_v2_cannot_reset_budget_after_any_predecessor_dispatch_or_reply(contract, field):
    item = contract.config["zero_output_predecessor"]["abort_receipt"]
    path = contract.root / item["path"]
    value = admission.strict_json(path.read_bytes())
    value[field] = 1
    raw = admission.canonical(value)
    path.write_bytes(raw)
    item.update(size_bytes=len(raw), sha256=admission.sha_bytes(raw))
    contract.sha = contract.save()
    with pytest.raises(admission.AdmissionError,
                       match="PREDECESSOR_NOT_ZERO_OUTPUT_TEMPLATE_ABORT"):
        execute(contract)


def test_v2_cannot_ignore_existing_predecessor_dispatch_journal(contract):
    item = contract.config["zero_output_predecessor"]["abort_receipt"]
    path = contract.root / item["path"]
    (path.parent / "01.dispatch.safe.json").write_text("{}", encoding="utf-8")
    with pytest.raises(admission.AdmissionError, match="PREDECESSOR_HAS_DISPATCH_JOURNALS"):
        execute(contract)


def test_v2_requires_pinned_zero_output_predecessor(contract):
    del contract.config["zero_output_predecessor"]
    contract.sha = contract.save()
    with pytest.raises(admission.AdmissionError, match="ZERO_OUTPUT_PREDECESSOR_REQUIRED"):
        execute(contract)
